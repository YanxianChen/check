...
public interface CheckRepository extends JpaRepository<Check, Long> {
	...
	//通过学号和课程号更改本次出勤记录
	@Modifying
	@Query("update Check u set u.attendence=?1 where u.sno=?2 and u.cno=?3")
	void updateAttendenceBySnoAndCno(int attendence,int sno,int cno);
	
	//通过学号和课程名更改出勤记录
	@Modifying
	@Query("update Check u set u.attendence=?1 where u.sno=?2 and u.cname=?3")
	void updateAttendenceBySnoAndCname(int attendence,int sno,String cname);
	...
	void updateAttendence(int sno,int cno);
